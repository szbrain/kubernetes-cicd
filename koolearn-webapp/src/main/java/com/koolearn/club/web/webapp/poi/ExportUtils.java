...
public class ExportUtils {
    ...
    public static <T extends Iterable, K extends Iterable> void export(String exportFile, String sheetName, Collection<String> titles, Collection<T> datas, Collection<K> footDatas, HttpServletResponse response){
        IExportBuilder sheetBuilder = new BaseExportBuilder(sheetName).buildTitle(titles).buildData(datas).buildFoot(footDatas);
        //导出excel文件
     ...
     * @param response
     */
    public static void export(String exportFile, IExportBuilder sheetBuilder, HttpServletResponse response){
    ...
    public static void export(String exportFile, IExportBuilder sheetBuilder){
     ...
     * @param response
     */
    public static void export(String exportFile, List<IExportBuilder> sheetBuilderList, HttpServletResponse response){
        if(sheetBuilderList == null || sheetBuilderList.size() == 0){
            return;
        }
        for (IExportBuilder builder : sheetBuilderList) {
            //定制Sheet
            buildSheet(builder);
    ...
    public static void export(String exportFile, List<IExportBuilder> sheetBuilderList){
...
//            System.out.println("====== >> use time = " + (endTime - startTime) + "ms");
     ...
     * @param sheetBuilder
     */
    private static void buildSheet(IExportBuilder sheetBuilder){
